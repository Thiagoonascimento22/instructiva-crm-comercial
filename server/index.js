...
import fs from "fs";
import { instalarSincronizacaoCRM } from "./sincroniza-crm.js";
...
// VENDAS (aba Vendas)
// ---------------------------------------------------------------------------
/* ---- integração com o Sistema Comercial (CRM): toda venda daqui vai pra lá ---- */
const crmSync = instalarSincronizacaoCRM({ app, getDb: () => db, saveDB, requireAuth });
const nomeDoVendedor = (id) => { const u = db.users.find((x) => x.id === id) || {}; return (u.nome || "").trim() || (u.login || "").trim() || "Suporte"; };
...
app.post("/api/vendas", requireAuth, (req, res) => {
  ...
  saveDB(db);
  // manda pro CRM (não trava a resposta: se der erro, entra na fila e reenvia sozinho)
  crmSync.enviarVenda(venda, nomeDoVendedor(vendedorId)).catch(() => {});
  res.json({ venda });
});
...
app.put("/api/vendas/:id", requireAuth, (req, res) => {
  ...
  saveDB(db);
  crmSync.enviarVenda(venda, nomeDoVendedor(venda.vendedorId)).catch(() => {});
  res.json({ venda });
});
