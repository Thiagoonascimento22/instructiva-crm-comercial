    ...
    try {
      ...
      for (const c of convs) {
        ...
        for (const m of c.msgs.slice(-25)) {
          ...
          let txt;
          if (m.tipo === "ligacao" && m.ligacao) {
            const l = m.ligacao;
            const st = l.atendida === false ? "NÃO ATENDIDA (o vendedor TENTOU ligar, cliente não atendeu)" : (l.duracao ? "ATENDIDA — conversou " + l.duracao + "s por telefone" : "realizada");
            txt = "📞 LIGAÇÃO " + (l.direcao === "entrante" ? "RECEBIDA" : "FEITA PELO VENDEDOR") + " — " + st + (l.resumo ? ". Resumo do que foi falado na ligação: " + l.resumo : "");
          } else {
            txt = m.transcricao ? "[áudio] " + m.transcricao
              : (m.content || (m.template ? "[abertura: template de disparo em massa]" : (m.tipo === "audio" ? "[áudio sem transcrição]" : (m.tipo && m.tipo !== "text" ? "[" + m.tipo + "]" : ""))));
          }
          txt = String(txt || "").replace(/\s+/g, " ").slice(0, 400);
      ...
      const sistema = "Você é um analista de vendas sênior da Instructiva, especialista no método dos 7 Passos da Venda (padrão Conquer). Analise as conversas de WhatsApp de um vendedor e faça uma avaliação PROFUNDA, honesta e específica, sempre citando situações reais que viu nas conversas (nunca invente). Seja detalhado e direto — nada de análise genérica ou rasa.\n\n"
        ...
        + "LIGAÇÕES (MUITO IMPORTANTE): quando aparecer '📞 LIGAÇÃO FEITA PELO VENDEDOR', significa que o vendedor LIGOU pro cliente (não ficou só no WhatsApp) — isso é EXATAMENTE o comportamento certo, VALORIZE bastante. Se a ligação foi ATENDIDA e teve conversa, considere que a etapa avançou por telefone (mesmo que o WhatsApp não mostre o conteúdo). Se aparecer 'Resumo do que foi falado na ligação', USE esse resumo pra entender o que foi tratado e avalie a condução. Se a ligação foi 'NÃO ATENDIDA', reconheça que o vendedor TENTOU o contato (conta como esforço de follow-up, não como abandono). NUNCA critique um vendedor por 'não ter puxado pra ligação' se você VÊ ligações registradas na conversa dele.\n\n"
